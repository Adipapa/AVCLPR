...
app = FastAPI(title="AVCLPR AI Engine", version="0.8.0")
...
def health():
    ...
    return {"status": "ok" if ready else "degraded", "service": "ai-engine", "version": "0.8.0", "vehicle_model": vehicle, "plate_model": plate, "tracker": {"active_tracks": len(tracker._tracks)}, "event_pipeline": {"recent_dedup_keys": events.recent_keys()}, "camera_workers": {"count": cameras.count(), "items": cameras.status()}, "central_store": central.health(), "sync_queue": outbox.stats()}
...
def start_camera(request: CameraStartRequest, x_ai_service_token: str | None = Header(default=None)):
    ...
    config = CameraWorkerConfig(site_id=request.site_id, camera_id=request.camera_id, rtsp_url=request.rtsp_url, site_uuid=request.site_uuid, camera_uuid=request.camera_uuid, sample_every_n_frames=request.sample_every_n_frames)
...
async def process_event_frame(image: UploadFile = File(...), site_id: str = "SITE-01", camera_id: str = "CAM-01-01", site_uuid: str | None = None, camera_uuid: str | None = None, sync: bool = False, x_ai_service_token: str | None = Header(default=None)):
    require_service_token(x_ai_service_token)
...
def run_sync(limit: int = 100, x_ai_service_token: str | None = Header(default=None)):
    ...
    for item in outbox.pending(limit=max(1, min(limit, 1000))):
        if sync_one(item["event"]):
            outbox.acknowledge(item["row_id"]); synced += 1
        else:
            outbox.fail(item["row_id"], "Central persistence failed"); failed += 1
...
def list_watchlists(x_ai_service_token: str | None = Header(default=None)):
    require_service_token(x_ai_service_token); return {"count": len(watchlists.list()), "items": watchlists.list()}
...
def add_watchlist(plate: str, category: str = "GENERAL", description: str | None = None, x_ai_service_token: str | None = Header(default=None)):
    require_service_token(x_ai_service_token)
    try: return {"success": True, "item": watchlists.upsert(plate, category, description)}
...
def delete_watchlist(plate: str, x_ai_service_token: str | None = Header(default=None)):
    require_service_token(x_ai_service_token)
    if not watchlists.remove(plate): raise HTTPException(status_code=404, detail="Watchlist plate not found")
    return {"success": True}

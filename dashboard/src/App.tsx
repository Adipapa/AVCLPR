...
function UsersPage({token,sites,onError}:{token:string;sites:Site[];onError:(s:string)=>void}){
  const [items,setItems]=useState<any[]>([]); const [form,setForm]=useState({username:'',displayName:'',role:'OPERATOR',password:''});
  const load=async()=>{try{setItems((await api('/users',{},token)).users||[])}catch(e){onError(e instanceof Error?e.message:'Unable to load users')}};
  useEffect(()=>{load()},[]);

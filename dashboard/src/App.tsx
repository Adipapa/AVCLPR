  ...
  return <div className="dashboard-shell">
    ...
    <div className="main-shell">
      ...
      <main className="content">
        ...
        {route === '/users' && user && ['SUPER_ADMIN','NATIONAL_ADMIN'].includes(user.role) && <UsersPage token={token} sites={sites} onError={setError} />}
...
function UsersPage({token,sites,onError}:{token:string;sites:Site[];onError:(s:string)=>void}){
  const [items,setItems]=useState<any[]>([]); const [access,setAccess]=useState<Record<string,string[]>>({});
  const [form,setForm]=useState({username:'',displayName:'',role:'OPERATOR',password:''});
  const load=async()=>{try{const users=(await api('/users',{},token)).users||[];setItems(users);const pairs=await Promise.all(users.map(async(u:any)=>[u.id,(await api('/users/'+u.id+'/sites',{},token)).siteIds||[]] as const));setAccess(Object.fromEntries(pairs));}catch(e){onError(e instanceof Error?e.message:'Unable to load users')}};
  useEffect(()=>{load()},[]);
  const create=async(e:FormEvent)=>{e.preventDefault();try{await api('/users',{method:'POST',body:JSON.stringify(form)},token);setForm({username:'',displayName:'',role:'OPERATOR',password:''});load()}catch(e){onError(e instanceof Error?e.message:'Unable to create user')}};
  const toggle=async(u:any)=>{try{await api('/users/'+u.id,{method:'PATCH',body:JSON.stringify({active:!u.active})},token);load()}catch(e){onError(e instanceof Error?e.message:'Unable to update user')}};
  const assign=async(u:any,siteId:string)=>{try{const ids=access[u.id]||[];const next=ids.includes(siteId)?ids.filter(x=>x!==siteId):[...ids,siteId];setAccess({...access,[u.id]:next});await api('/users/'+u.id+'/sites',{method:'PUT',body:JSON.stringify({siteIds:next})},token);}catch(e){onError(e instanceof Error?e.message:'Unable to update site access');load()}};
  return <><PageHeader title="Users & Roles" subtitle="Administrative identity, role and site-access management."/><div className="card"><form className="inline-form" onSubmit={create}><input required placeholder="Username" value={form.username} onChange={e=>setForm({...form,username:e.target.value})}/><input required placeholder="Display name" value={form.displayName} onChange={e=>setForm({...form,displayName:e.target.value})}/><select value={form.role} onChange={e=>setForm({...form,role:e.target.value})}>{['NATIONAL_ADMIN','POLICE','TRANSPORT','PURA','GICTA','INTELLIGENCE','ANALYST','OPERATOR','VIEW_ONLY'].map(r=><option key={r}>{r}</option>)}</select><input required minLength={12} type="password" placeholder="Initial password" value={form.password} onChange={e=>setForm({...form,password:e.target.value})}/><button className="primary">CREATE USER</button></form></div><div className="card"><DataTable headers={['USERNAME','NAME','ROLE','MFA','ACTIVE','SITE ACCESS','ACTION']} rows={items.map(u=>[u.username,u.display_name,u.role_code,u.mfa_enabled?'ENABLED':'REQUIRED',u.active?'YES':'NO',<div>{sites.map(s=><label key={s.id} style={{display:'inline-flex',gap:4,marginRight:8}}><input type="checkbox" checked={(access[u.id]||[]).includes(s.id)} onChange={()=>assign(u,s.id)}/>{s.site_code}</label>)}</div>,<button onClick={()=>toggle(u)}>{u.active?'DISABLE':'ENABLE'}</button>])}/></div></>}

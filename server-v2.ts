...
app.get('/api/v2/audit',requireAuth,requirePermission('audit.read'),async(req:AuthenticatedRequest,res)=>res.json({success:true,...await listAuditLogs({limit:req.query.limit,offset:req.query.offset,userId:req.user!.id,role:req.user!.role})}));
